...
class TSne():
    ...
    verbose : int, default=0
        ...
        0 for no info, 1 for total execution time and time/iteration, 2 for every *iters_check* iterations
        TODO: explicarlo
    ...
    def __init__(self, *, n_components=2, perplexity=30., perplexity_tolerance=1e-2,
        ...
        self._n_components = n_components if isinstance(n_components, int) else int(np.floor(n_components))
        ...
        #=== Plotting Params
        self._plotting_fig, self._plotting_ax = plt.subplots() if self._n_components==2 else plt.subplots(subplot_kw=dict({"projection": "3d"}))
    ...
    def __init_validation(self,n_components,perplexity,perplexity_tolerance,metric,init,
        ...
        # N dimensions: int
        _assert_input("n_components", n_components, "int", less=1)
        if n_components is not None and n_components>3:
        ...
        if init is not None:
            if isinstance(init, str):
                ...
                if n_components is not None:
                    assert init.shape[1]==n_components, "The initial embedding must have the number of dimensions provided"
            else:
        ...
        # Max iter: int
        _assert_input("n_iter", n_iter, "int", more_equal=1)
    ...
    def __input_validation(self, input, labels=None):
        ...
        if labels is not None:
            ...
            assert plotting_labels.ndim==1, "labels must be a 1D array"
            
            marker_options = ['o','*','v','^','<','>']
            if np.all(np.char.isnumeric(plotting_labels)):
                self._plotting_labels = np.array(labels, dtype=int)
                self._plotting_colors = np.array(labels, dtype=int)
                self._plotting_markers = np.empty_like(plotting_labels)
    ...
    def fit(self, input, labels=None, record_embed=False, record_cost=False, gif_filename=None, gif_kwargs=None) -> np.ndarray:
        ...
        self._init_embed = self.__rand_embed(X, self._n_components)
        ...
        self._embed = self._init_embed.copy()

        if record_embed:
            self._embedding_record.append(self._embed.copy())
        if record_cost:
            self._cost_record = dict()
        ...
        ani = animation.FuncAnimation(self._plotting_fig, self.__update_anim, self._n_iter, fargs=[p, self._plotting_ax, record_embed, record_cost], interval=100, repeat=False)
        ...
        if self._verbose>0:
            ...
            tiempos = np.array([t, t/self._n_iter])
    ...
    def __update_embed(self, i, affinities, save_cost=False):
        embed_dist = similarities.pairwise_euclidean_distance(self._embed)
        ...
        if i%self._iters_check==0:
            ...
            if save_cost:
                self._cost_record.update({i: cost})

        # Calculo de nuevo embed
        grad = gradient(p, q, self._embed, embed_dist)
        self._update = momentum*self._update - grad*self.__lr
        self._embed += self._update


    def __update_anim(self, i, affinities, ax:Axes, save_record, save_cost):
        self.__update_embed(i, affinities, save_cost)
        if save_record:
            self._embedding_record.append(self._embed.copy())
        ...
        # Title
        plt.title("Current Iteration: {}/{} \n Best cost: i={}, cost={:.3f}".format(i+1, self._n_iter, self._best_iter, self._best_cost))

    def get_embed_record(self) -> None | list[np.ndarray]:
        """Returns a list of the embeddings obtained in the process
        if fit(input) has been called with record_embed==True.
        
        Otherwise, returns None.
        """
        return self._embedding_record
    def get_cost_record(self) -> None | dict[int, float]:
        """If fit(input) has been called with record_cost==True,
        returns a dictionary made from the pairs [i, C(i)], where C(n)
        is the value of the cost function in the n-th iteration of the algorithm.
        
        
        Otherwise, returns None.
        """
        return self._cost_record
